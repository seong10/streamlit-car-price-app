...
def run_eda() :
    ...
    st.dataframe(df_min)
    # 컨트롤 슬래쉬 = 주석
    #elif selected == radio_menu[2] :
    #    car_max = car_df.loc[ car_df['Annual Salary'] == car_df['Annual Salary'].max() , ]
    #    st.dataframe( car_max )
    ...
    # if len( str(input) ) != 0 :
    # 1. 유저한테 검색어를 입력받는다.
    input = st.text_input('이름을 입력하세요')
    # 2. 검색어를 고객이름 컬럼에 들어있는 데이터 가져온다.
    result = car_df.loc[ car_df['Customer Name'].str.lower().str.contains(input.lower()) , ]
    # 3. 화면에 보여준다.
    st.dataframe(result)